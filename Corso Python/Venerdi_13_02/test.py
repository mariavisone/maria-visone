''' python è un linguaggio di programmazione ovvero un linguaggio che contiene comandi di controllo e struttura. 
In specifico: python è un linguaggio interpretato, orientato agli oggetti, ad alto livello, dinamicamente tipizzato, semplice.
si definisce interpretato perchè il programmatore scrive il codice e la macchina lo traduce, se è correttamente scritto
E' orientato agli oggetti in quanto deve ripettare le 3 regole dell'OOP ovvero incapsulamento (per la protezione dati), 
ereditarietà(riutilizzo del codice in altre classi) e polimorfismo(flessibilità)
...
una caratteristica unica di python è l'indentazione ovvero lo spazio tra margine e riga di codice; 
ogni volta che ho as esmepio un nuovo blocco di codice devo aumentare il livello di indentazone 
per usare python dobbiamo istallare un IDE ovvero un programma che ci aiuta
a sceivere il codice tutto in un unico ambiente chje è isolato dal sistema operativo o da altri programmi. 
quest'isolamento avviene perchè specifacemnte nell'IDE si va a scaricare l'estenzione di python come sistema integrato e si lavora li. 
in python si utilizzano funzioni incorporate ovvero print() input() len()  ovvero che possono essere utilizzare senza richiami esterni. 
E in python, fondamentali per la programmazione sono le variabili che possono essere di tipo basilari e non 
(rispettivaemnte che sono già esistenti all'interno di python e oggetti che invece creo io come liste e tuple, insiemi) '''




# 5
una collezione è un insieme di variabili salvate in unico punto in memoria. per i programmatori sono molto utili. 
ci sono 4 tipi di collezioni: liste, tuple, insiemi e dizionari
questi possoono essere modificabili, non modificabili e dinamiche.
le liste sono le strutture dati piu utilizzate. sono collezioni ordinate, modificabili e eterogenee.
vengono definite con il tipo list con le parentesi []. ad esempio: listaX = [1, 2, "Marco", 300,]
In una list aposso aggiungere, rimuovere o sostituire un valore grazie agli indici di posizione e con funzioni specifiche. 
aggiungere con .append(), rimuovere con .remove() o sostituire indicando l'indice di posizione
...
un insieme invece rappresenta una struttura di dati, ovvero una collezione non ordinata,modificabile e può essere mista. 
il tipo è set con {}. ex. set1 = {1, 2, 4, 4} 
un dizionario invece è una struttura dati che permette di memorizzare i dati sottoforma di chiavi e valori. 
sono utili per creare oggetti e tabelle ad esempio
può essere misto in parte ovvero semieterogeneo, ordinato e modificabile. il tipo è dict con {chiave : valore }
la chiave è sempre una stringa mentre il valore può essere una stringa o un numero. 
si può accdere ai valori con la chiave corrispondente. ex. di dizionario dove la chiave è il nome e il valore è l'età: dict ={"nome": 3, ..}

#7
una classe è una struttura che raggruppa oggetti. 
in specifico è un disegno tecnico dell'oggett oche vogliamo creare, il suo blueprint.
possiamo avere il nome di una classe ma una classe può avere infiniti oggetti all'interno.'
'ogni classe può avere: variabili e attributi, metodi semplici, metodi speciali e altre classi all'interno. 
gli attributi in specifico sono le caratteristiche dell'oggetto mentre i metodi sono le funzioni associate alla classe che operano sugli oggetti 
(chiamati anche istanze) e possono essere di 3 tipi: metodi statici, metodi decorati e metodi di istanza.
i metodi speciali che conosciamo sono: il costruttore e  il tool string.
il costruttore veien invocato automaticamente al momento della creazione dell'oggetto ed è rappresetnato con __init__ e serve a creare e iniziare l'oggetto, e ha bisogno del parametro self. 
